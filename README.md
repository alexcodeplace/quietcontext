...
qc repo explore "how does MyType save records?"
qc repo map
...
Structural user prompts can also be front-loaded automatically through the existing `UserPromptSubmit` hook. The hook uses a cheap multilingual/code-shaped classifier; eligible prompts make one authenticated request to the shared local QC daemon with a 650 ms fail-open deadline and inject at most 4 KiB of compact `repo explore` context. Cold roots may inject nothing rather than delaying the agent. Non-structural prompts make no QC request. This mechanism does **not** deny or rewrite Read/Grep, and it is independent of Bash routing. Set `QUIET_CONTEXT_FRONTLOAD_DISABLE=1` to disable prompt front-loading for diagnosis.

`qc run -- ...` executes argv directly through the same native filtering engine used by supported MCP shell commands. It preserves the child exit code. When filtering omits raw text, QuietContext retains bounded exact evidence and indexes searchable text so `search` can recover an omitted line. `qc repo` exposes the same native repository engine as the MCP `repo` tool. `qc explore` ranks likely symbols/files directly from the structural index, returns one bounded source excerpt plus compact related candidates, and reuses callers/callees/impact only when that semantic generation is already warm. It never forces a cold semantic build. `qc map` stays a compact orientation view; JavaScript/TypeScript, Python, Rust, and Go are also indexed into a bounded semantic graph for resolved references, callers, callees, impact, dependencies, dependents, and shortest dependency paths. Ambiguous same-named definitions are kept separate rather than guessed, and `--file` can narrow a CLI symbol query. The compact MCP `repo` surface uses `symbol@path` to disambiguate colliding definitions and `from -> to` as the `path` target. `qc index` and `qc search` are local front doors to QuietContext's project-scoped FTS5 store; `qc index --stdin` accepts at most the normal per-source indexing cap and rejects invalid UTF-8 instead of silently indexing binary data.
