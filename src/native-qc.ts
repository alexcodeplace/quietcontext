...
export interface QcNativeRepoReceipt {
  ...
  nativeVersion: string;
  kind: "repo";
  operation: "frontload" | "explore" | "map" | "sym" | "refs" | "outline" | "callers" | "callees" | "impact" | "deps" | "dependents" | "path";
  root: string;
  query?: string;
  stdout: string;
  stderr: string;
...
export async function repoQcNative(
  request:
    ...
    | { action: "frontload" | "explore"; query: string; root?: string }
  ...
  if (request.action === "frontload" || request.action === "explore" || request.action === "symbol" || request.action === "references") args.push(request.query);
