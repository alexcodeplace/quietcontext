...
function seedContentStore(path: string, label: string): void {
  const store = new ContentStore(path);
  store.index({ content: `${label} payload`, source: label });
  store.close();
}
...
describe("purgeSession — slice 1: session events DB (canonical)", () => {
  it("logically clears a live database without detaching peers", () => {
    ...
    const peer = new SessionDB({ dbPath });
    first.ensureSession("session-one", projectDir);
    expect(peer.getSessionStats("session-one")).not.toBeNull();

    const r = purgeSession({ projectDir, sessionsDir });

    expect(existsSync(dbPath)).toBe(true);
    expect(first.getSessionStats("session-one")).toBeNull();
    expect(peer.getSessionStats("session-one")).toBeNull();
    const fresh = new SessionDB({ dbPath });
    expect(fresh.getSessionStats("session-one")).toBeNull();
    expect(r.deleted).toContain("session events DB");
    expect(r.wipedPaths).not.toContain(dbPath);
    fresh.close();
    peer.close();
    first.close();
  });

  it("removes a corrupt database without retaining a recovery copy", () => {
    const projectDir = makeRepo("s1-corrupt");
    const sessionsDir = makeTmpDir("sess1-corrupt");
    const suffix = getWorktreeSuffix(projectDir);
    const canonicalHash = hashProjectDirCanonical(projectDir);
    const dbPath = join(sessionsDir, `${canonicalHash}${suffix}.db`);
    writeFileSync(dbPath, "not a sqlite database");

    const result = purgeSession({ projectDir, sessionsDir });

    expect(result.deleted).toContain("session events DB");
    expect(existsSync(dbPath)).toBe(false);
    expect(readdirSync(sessionsDir).some((file) => file.includes(".corrupt-"))).toBe(false);
  });

  it("fails within the contention bound instead of reporting a false purge", () => {
    const projectDir = makeRepo("s1-lock");
    const sessionsDir = makeTmpDir("sess1-lock");
    const suffix = getWorktreeSuffix(projectDir);
    const canonicalHash = hashProjectDirCanonical(projectDir);
    const dbPath = join(sessionsDir, `${canonicalHash}${suffix}.db`);
    const session = new SessionDB({ dbPath });
    session.ensureSession("locked-session", projectDir);
    session.close();
    ...
    try {
      expect(() => purgeSession({ projectDir, sessionsDir })).toThrow(/SQLITE_BUSY|database is locked/i);
      expect(Date.now() - startedAt).toBeLessThan(750);
    } finally {
      locker.exec("ROLLBACK");
      locker.close();
    }
    const inspect = new SessionDB({ dbPath });
    expect(inspect.getSessionStats("locked-session")).not.toBeNull();
    inspect.close();
  });
});
...
describe("purgeSession — slice 5: knowledge base FTS5 store", () => {
  it("logically clears a shared store while peer connections remain open", async () => {
    ...
    const first = new ContentStore(storePath);
    first.index({ content: "shared project payload", source: "shared-source" });
    const peer = new ContentStore(storePath);

    const r = purgeSession({ projectDir, sessionsDir, storePath });

    expect(existsSync(storePath)).toBe(true);
    expect(first.listSources()).toEqual([]);
    expect(peer.listSources()).toEqual([]);
    const fresh = new ContentStore(storePath);
    expect(fresh.listSources()).toEqual([]);
    fresh.close();
    peer.close();
    first.close();
    expect(r.deleted).toContain("knowledge base (FTS5)");
    expect(r.wipedPaths).not.toContain(storePath);
  });

  it("removes a corrupt content store without retaining recovery bytes", () => {
    const projectDir = makeRepo("s5-corrupt");
    const sessionsDir = makeTmpDir("sess5-corrupt");
    const contentDir = makeTmpDir("content5-corrupt");
    const storePath = join(contentDir, "corrupt.db");
    touchSqliteTriple(storePath);

    const result = purgeSession({ projectDir, sessionsDir, storePath });
    ...
    expect(readdirSync(contentDir).some((file) => file.includes(".corrupt-"))).toBe(false);
  });

  it("fails content purge within the contention bound", async () => {
    const projectDir = makeRepo("s5-lock");
    const sessionsDir = makeTmpDir("sess5-lock");
    const contentDir = makeTmpDir("content5-lock");
    const storePath = join(contentDir, "locked.db");
    const { ContentStore } = await import("../../src/store.js");
    const store = new ContentStore(storePath);
    store.index({ content: "locked project payload", source: "locked-source" });
    store.close();
    const Database = loadDatabase();
    const locker = new Database(storePath);
    locker.exec("BEGIN IMMEDIATE");
    const startedAt = Date.now();
    try {
      expect(() => purgeSession({ projectDir, sessionsDir, storePath })).toThrow(/SQLITE_BUSY|database is locked/i);
      expect(Date.now() - startedAt).toBeLessThan(750);
    } finally {
      locker.exec("ROLLBACK");
      locker.close();
    }
    const inspect = new ContentStore(storePath);
    expect(inspect.listSources().map((source) => source.label)).toContain("locked-source");
    inspect.close();
  });
...
describe("purgeSession — slice 6: legacy shared content store", () => {
  it("logically clears the legacy content database", () => {
    ...
    seedContentStore(legacyDb, "legacy-content");

    purgeSession({ projectDir, sessionsDir, legacyContentDir, contentHash });

    expect(existsSync(legacyDb)).toBe(true);
    const inspect = new ContentStore(legacyDb);
    expect(inspect.listSources()).toEqual([]);
    inspect.close();
  });
...
describe("purgeSession — slice 8: worktree separation", () => {
  it("only wipes files for THIS projectDir's hash; sibling worktree DB untouched", () => {
    ...
    const first = new SessionDB({ dbPath: db1 });
    const sibling = new SessionDB({ dbPath: db2 });
    first.ensureSession("first-session", wt1);
    sibling.ensureSession("sibling-session", wt2);
    first.close();
    sibling.close();
    ...
    // wt1 — database retained and rows cleared
    expect(existsSync(db1)).toBe(true);
    const firstAfter = new SessionDB({ dbPath: db1 });
    expect(firstAfter.getSessionStats("first-session")).toBeNull();
    firstAfter.close();
    expect(existsSync(events1)).toBe(false);
    expect(existsSync(flag1)).toBe(false);
    // wt2 — UNTOUCHED
    expect(existsSync(db2)).toBe(true);
    const siblingAfter = new SessionDB({ dbPath: db2 });
    expect(siblingAfter.getSessionStats("sibling-session")).not.toBeNull();
    siblingAfter.close();
...
describe("purgeSession — slice 10: FTS5 dual-hash sweep via contentDir", () => {
  it.skipIf(process.platform === "linux")(
    ...
    () => {
      ...
      seedContentStore(canonicalDb, "canonical");
      seedContentStore(legacyDb, "legacy");
  ...
  it.skipIf(process.platform !== "linux")(
    ...
    () => {
      ...
      seedContentStore(dbPath, "linux-content");
      const r = purgeSession({ projectDir, sessionsDir, contentDir });
      expect(existsSync(dbPath)).toBe(true);
      expect(r.deleted).toContain("knowledge base (FTS5)");
    },
  ...
  it("worktree separation: sibling project's FTS5 file at different hash is untouched", () => {
    ...
    seedContentStore(wt1Db, "worktree-one");
    seedContentStore(wt2Db, "worktree-two");

    purgeSession({ projectDir: wt1, sessionsDir, contentDir });

    expect(existsSync(wt1Db)).toBe(true);  // logically cleared
...
describe("purgeSession — slice 9: backward-compatible labels", () => {
  it("uses the exact legacy labels: 'knowledge base (FTS5)', 'session events DB', 'session events markdown'", () => {
    ...
    seedContentStore(storePath, "labels-content");
    const sessionDbPath = join(sessionsDir, `${canonicalHash}${suffix}.db`);
    const sessionDb = new SessionDB({ dbPath: sessionDbPath });
    sessionDb.ensureSession("labels-session", projectDir);
    sessionDb.close();
...
describe("purgeSession — issue #520 slice 3: per-session FTS5 chunk wipe", () => {
  it("deletes chunks where session_id = sessionId; sibling rows preserved", async () => {
    ...
    const source = checkAfter.prepare(
      "SELECT chunk_count, code_chunk_count, indexed_bytes FROM sources WHERE id = ?",
    ).get(srcId) as { chunk_count: number; code_chunk_count: number; indexed_bytes: number };
    const totals = checkAfter.prepare(
    ...
    expect(source).toEqual({ chunk_count: 1, code_chunk_count: 1, indexed_bytes: 11 });
    expect(totals).toEqual({ indexed_bytes: 11, chunks: 1, sources: 1 });
...
describe("purgeSession — issue #520 slice 2: per-session DB row wipe", () => {
  it("wipes target session_events rows; sibling session preserved", async () => {
    ...
    seed.incrementToolCall("scratch", "Read", 100);
    seed.incrementToolCall("main", "Read", 50);
    const staleWriter = new SessionDB({ dbPath });
    seed.close();

    purgeSession({ projectDir, sessionsDir, scope: "session", sessionId: "scratch" });

    expect(() => staleWriter.insertEvent(
      "scratch",
      { type: "file", category: "file", data: "/scratch/stale.ts", priority: 2 },
      "PreToolUse",
      undefined,
      undefined,
      0,
    ...
    expect(verify.getToolCallStats("scratch").totalCalls).toBe(0);
    expect(verify.getToolCallStats("main").totalCalls).toBe(1);
    verify.close();
