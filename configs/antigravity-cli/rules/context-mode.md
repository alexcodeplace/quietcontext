...
Do NOT use `context-mode/exec-file` or `execute` to print a whole file
...
- WRONG: `exec-file(path:"config.yaml", language:"python", code:"print(FILE_CONTENT)")`
...
- `ToolName`: `"execute"`, `"exec-file"`, `"batch"`,
  `"fetch-index"`, `"search"`, or `"index"`
...
- `context-mode/execute`: `{"language":"python","code":"..."}`
- `context-mode/exec-file`:
  `{"path":"path/to/file","language":"python","code":"..."}`
- `context-mode/batch`:
  `{"commands":[{"label":"...","command":"..."}],"queries":["..."]}`
- `context-mode/fetch-index`: `{"url":"https://...","source":"..."}`
- `context-mode/search`: `{"queries":["q1","q2"]}`
- `context-mode/index`: `{"path":"path/to/file-or-dir","source":"..."}`
  or `{"content":"...","source":"..."}`

## Mandatory routing

- Think in code. For analyze/count/filter/compare/search/parse/transform tasks,
  write code with `context-mode/execute` and print only the final answer.
  Program the analysis; do not read raw data and compute mentally.
- File read for analysis: there is no separate `ctx_read` tool.
  `context-mode/exec-file` is the context-mode file-read surface. It
...
- Native `Read` / `view_file` is correct when editing requires exact bytes, or
  ...
  `context-mode/exec-file`.
- Use one `context-mode/batch` call for multi-command repository
  reconnaissance. One batch should replace many shell/list/search calls.
- Use `context-mode/execute` for shell commands whose output may exceed a
  short fixed answer. Native Bash is only for git, mkdir, rm, mv, navigation,
  installs, or short observable output.
- Use `context-mode/fetch-index` for web content, then
  `context-mode/search` to query it. Do not dump raw HTML into the
  conversation.
- Use `context-mode/index` only when content should be stored and searched
  later. For a one-off file question, prefer `context-mode/exec-file`;
